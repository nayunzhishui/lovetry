...
const archiveAccessSource = path.join(cloudRoot, "_shared", "archive-access.js");
const archiveAccessTargets = ["couple", "dashboard"];
...
for (const target of archiveAccessTargets) {
  fs.copyFileSync(archiveAccessSource, path.join(cloudRoot, target, "archive-access.js"));
}

console.log(`已同步 ${sourceFiles.length} 个共享模块到小程序运行时目录。`);
console.log(`已同步 membership resolver 到 ${membershipTargets.length} 个云函数目录。`);
console.log(`已同步 relationship archive access 到 ${archiveAccessTargets.length} 个云函数目录。`);
