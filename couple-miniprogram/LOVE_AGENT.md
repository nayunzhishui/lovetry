...
Agent 默认不读取记录。用户可以主动打开“双轨授权单”，只从本人创建的生活、心情和沟通记录中选择一条；健康记录和伴侣创建的记录不进入候选。页面展示准确预览，内容最多 400 字，仅用于下一次回答，发送后自动移除。服务端再次限制类型和长度，并把记录视为可能不完整的单方背景，而不是系统指令或事实全貌；临时记录中的危险信号同样进入安全分流。

每条回答可以交接到沟通表单：先在本机保存 30 分钟的短期建议，再由用户在表单中二次确认。导入后强制为仅自己可见的草稿，用户需要核对并改成自己的话；系统不会自动发送、共享或创建共同约定。
...
- 只有用户明确选择后，才发送一条本人记录的脱敏预览；不发送记录 ID、伴侣记录或健康记录；
- API 密钥仅保存在云函数环境变量。
...
当前内容版本记录在 `knowledge-manifest.json`：36 条内部审查知识，稳定编号为 K01～K36。达到持证专业人员复核条件后，再扩充到 80～150 条；约 200 条以上才评估向量检索。

## 7. 上线前评测集

`evals/love-agent-scenarios.json` 当前包含 28 个场景、112 条独立问题，明确覆盖普通关系沟通、即时暴力/性强迫、自伤和强迫控制。`npm run eval:agent` 必须达到安全分流与知识检索全部通过；这只验证确定性前置链路，不替代真实模型人工评分。上线前还要覆盖：
