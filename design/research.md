...
## P13 十四项功能与参考视频研究（2026-07-20）

### 本地参考视频观察

- 生理期产品：月历、开始记录、历史周期和周期长度统计形成单一闭环；可借鉴透明的历史与估算，不复制生育概率、会员锁定和医学承诺；
- 健康记录产品：睡眠以入睡—醒来时间轴呈现，心情/压力提供日周月趋势；本项目采用近 7 条轻量趋势，避免制造临床监测感；
- 两种记账产品：都将总资产、账户、流水和周/月/年统计分层；奖励账户对应“双方钱包—余额/累计—流水”，不引入债务或消费攀比；
- 共通模式：首屏先摘要，再给主动作；列表可按时间/类型筛选；图表用少量颜色和直接标签，不让用户依赖图例猜含义。

### GitHub 与公开项目证据

- `period-tracker` 使用本地优先、平均值/标准差和异常值处理表达透明周期预测；本项目进一步采用更稳健的中位数，并明确样本数与非医疗边界：<https://github.com/krithravi/period-tracker>；
- `obsidian-periodic-notes-calendar` 将日历、周期性内容和预览放在移动端连续流程中，支持本项目“选日期后原地查看与新增”：<https://github.com/luiisca/obsidian-periodic-notes-calendar>；
- `obsidian-day-planner` 连接可编辑日历、任务与时间追踪，支持计划—日历—番茄钟的关联：<https://github.com/ivan-lednev/obsidian-day-planner>；
- `PomodoroLM` 提供可配置专注/休息、标签、自动开始和清晰进度；本项目采用可配置与恢复，不默认强提醒：<https://github.com/vjousse/pomodorolm>；
- 微信官方 WeUI 提供原生小程序表单、反馈、空态和深色模式参考；本项目只吸收交互规范，保留自有美术：<https://github.com/wechat-miniprogram/weui-miniprogram>；
- TDesign 提供多端设计系统及小程序实现，支持统一状态、尺寸与组件语义：<https://github.com/Tencent/tdesign>；
- `Birday` 证明提醒产品应保持轻量、明确和专注，适合纪念日与待办的克制呈现：<https://github.com/m-i-n-a-r/birday>。

### 采用与边界

- 采用：摘要优先、日期原地录入、轻量趋势、透明估算、账户—流水层级、绝对时间计时；
- 不采用：生育概率、伴侣监控、情绪诊断、睡眠医疗建议、消费排行、胜负排行和自动分享身体数据；
- 许可证策略：只参考交互模式与公开文档；未确认兼容许可证的项目不复制代码或资源。
